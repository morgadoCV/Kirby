...
//Constructor
Particle::Particle(fPoint respawn, iPoint area, iPoint timelife, fPoint speed_particle, P_Direction p_direction, SDL_Rect pos_rect, int size, int num_tex_pixel, bool active, iPoint num_tex)
{
...
void Particle::render(int scale)
{
	...
	App->render->Blit(App->particles->atlas_particle, position.x / scale, position.y / scale, &rect, scale);
}

void Particle::Move(fPoint speed, bool Move_alternative)
{
